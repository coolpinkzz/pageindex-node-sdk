...
### Local testing as a dependency

You can exercise the package locally without publishing:
...
mkdir /tmp/pageindex-test && cd /tmp/pageindex-test
npm init -y
npm install /path/to/pageindex-node-sdk-0.1.0.tgz
```
...
const res = await client.listDocuments({ limit: 1, offset: 0 });
console.log(res);
```
...
```bash
PAGEINDEX_API_KEY=your_key_here node test.mjs
```
...
- This is an community SDK, not maintained by the PageIndex team.
- For full API details, always refer to the official docs: <https://docs.pageindex.ai/>.
